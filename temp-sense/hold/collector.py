...
UDP protocol (see temp-logger-udp-protocol.md) and insert them into a
sqlite db.
...
Usage:
    ./collector.py                       # pull into ./temp_data.db
    ./collector.py --db /path/to.db
...
deduped by (romcode, timestamp_epoch) via the `readings` table's primary
key, so re-pulled data is a no-op.
...
Note: a temp_data.csv written by a pre-sqlite version of this script is
not read or migrated by this version -- it's a separate, unrelated file.
"""
...
def load_table(path):
    """sensor_id (int) -> (romcode, label), from a --table refresh. Raises
    if the file doesn't exist -- a normal pull must not guess at identities
    it hasn't been told."""
...
def write_sets(sets, cur, table):
    """INSERT OR IGNORE relies on the `readings` table's
    (romcode, timestamp_epoch) primary key to skip a re-pulled packet
    (retry, or a whole session re-offered after a prior abandoned run)
    rather than duplicating it. Keyed by romcode rather than label: the
    device assigns every newly-registered sensor the same placeholder
    label ("unlabeled") until an operator names it, so two such sensors
    active at once would otherwise collide on (label, timestamp_epoch) and
    silently lose one of the two readings every cycle."""
        ...
        for sensor_id, raw, valid in readings:
            if sensor_id not in table:
                raise RuntimeError(
                    ...
                    .format(sensor_id))
            romcode, label = table[sensor_id]
            iso = datetime.datetime.fromtimestamp(
                timestamp, tz=datetime.timezone.utc).isoformat()
            cur.execute(
                "INSERT OR IGNORE INTO readings "
                "(timestamp_epoch, romcode, label, timestamp_utc, temp_c, "
                "valid) VALUES (?, ?, ?, ?, ?, ?)",
                (timestamp, romcode, label, iso, raw / 16.0, valid))
...
def run_transfer(sock, addr, max_retries, retry_interval_s, cur, table):
    expected_seq = 0
    transfer_id = None
...
def main():
    ...
    ap.add_argument("--db", default="temp_data.db",
    ...
    conn.execute(
        ...
        "timestamp_epoch INTEGER, romcode TEXT, label TEXT, "
        "timestamp_utc TEXT, temp_c REAL, valid INTEGER, "
        "PRIMARY KEY (romcode, timestamp_epoch))")
    cur = conn.cursor()
    ok = run_transfer(sock, addr, max_retries, retry_interval_s, cur, table)
    conn.close()
    sock.close()
